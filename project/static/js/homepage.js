$(document).ready( function () {
    ...
    document.getElementById('search_room').addEventListener('click', function () {
        ...
        var checkout_date_value = document.getElementById('text_checkout_date').value;
        ...
        var reservationURL = '/reserve?checkin=' + checkin_date_value + '&checkout=' + checkout_date_value + '&adult=' + adult + '&child=' + child;
...
function set_default_date() {
    todayDate = new Date();
    document.getElementById('text_checkin_date').valueAsDate = new Date(todayDate);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('text_checkin_date').setAttribute('min', current_date);
    document.getElementById('text_checkout_date').setAttribute('min', formatted_date);

    document.getElementById('text_checkin_date').addEventListener('input', function () {
        ...
        var current_checkout_date = new Date(document.getElementById('text_checkout_date').value);
        ...
        if (current_checkout_date < min_checkout_date) {
            ...
            document.getElementById('text_checkout_date').value = min_checkout_date.toISOString().split('T')[0];
        }

        // อัพเดท min attribute ของ "เช็คเอาท์" เพื่อให้ไม่สามารถเลือกวันที่ก่อน "เช็คอิน" ได้
        document.getElementById('text_checkout_date').setAttribute('min', min_checkout_date.toISOString().split('T')[0]);
    });
});
